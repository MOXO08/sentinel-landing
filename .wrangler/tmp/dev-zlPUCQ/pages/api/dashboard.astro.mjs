...
const POST = async ({ request, locals, url: reqUrl }) => {
  ...
  if (path === "/api/dashboard/api/rotate") {
    ...
    if (env.DB) {
      await env.DB.prepare("UPDATE clients SET api_key = ? WHERE id = ?").bind(newKey, client.id).run();
    }
...
const GET = async ({ request, locals, url: reqUrl }) => {
  const env = locals.runtime.env;
  const url = new URL(request.url);
  const path = url.pathname;
